...
from sanchit.modules.telegram import get_message, get_file_properties
...
async def transmit_file(file_id):
    ...
    async def file_generator():
        current_part = 1
        async for chunk in Sanchit.iter_download(file, offset=offset, chunk_size=chunk_size, stride=chunk_size, file_size=file_size):
            if not chunk:
                break
            elif part_count == 1:
