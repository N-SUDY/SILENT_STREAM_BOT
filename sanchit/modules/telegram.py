...
from sanchit.server.error import abort

async def get_message(message_id: int) -> Message | None:
    message = None
    
    try:
        message = await Sanchit.get_messages(Telegram.CHANNEL_ID, ids=message_id)
    except Exception:
        pass
...
async def send_message(message:Message, send_to:int = Telegram.CHANNEL_ID) -> Message:
    return await Sanchit.send_message(entity=send_to, message=message)
